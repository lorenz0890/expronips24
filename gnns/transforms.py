...
class WLColorAndConnect(BaseTransform):
    def __init__(self, num_iterations=5):
        super(WLColorAndConnect, self).__init__()
        self.num_iterations = num_iterations  # Number of WL iterations

    def __call__(self, data):
        x, edge_index = data.x, data.edge_index

        # Convert binary vectors to integers
        colors = binary_to_int(data.x).long()

        wl = torch_geometric.nn.conv.WLConv()
        # WL iterations to refine colors
        for _ in range(self.num_iterations):
            # Create a hash value for each node based on its color and the colors of its neighbors
            colors = wl(colors, edge_index)

        # Connect nodes with the same color
        color_to_nodes = {}
        for i, color in enumerate(colors):
            if color.item() not in color_to_nodes:
                color_to_nodes[color.item()] = []
            color_to_nodes[color.item()].append(i)

        new_edges = []
        for nodes in color_to_nodes.values():
            # Connect all nodes within the same color group
            for i in range(len(nodes)):
                for j in range(i + 1, len(nodes)):
                    new_edges.append([nodes[i], nodes[j]])

        # Ensure the new edges are integral and concatenate with the original edge_index
        new_edge_index = torch.tensor(new_edges, dtype=torch.long).t().contiguous()
        data.edge_index = to_undirected(torch.cat([edge_index, new_edge_index], dim=1), num_nodes=data.num_nodes)
